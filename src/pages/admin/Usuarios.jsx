...
export default function Usuarios() {
  ...
  const rolesPermitidos = roles.filter((rol) => ROLES_CREABLES.includes(rol.nombre));
  const rolSeleccionado = rolesPermitidos.find(
    (rol) => rol.idRol.toString() === form.idRol.toString()
  );
  ...
  useEffect(() => {
    ...
    Promise.all([listarUsuarios(), listarRoles(true)])
      ...
      .catch((err) => {
        if (montado) setError(extraerMensajeError(err));
      })
      .finally(() => {
  ...
  const usuariosFiltrados = usuarios.filter((usuario) => {
    const texto = `${usuario.correo} ${usuario.nombreRol}`.toLowerCase();
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editando) {
        ...
        if (!rolSeleccionado) {
          throw new Error("Solo se pueden crear usuarios RECEPCIONISTA o DOCTOR.");
        }

        if (rolSeleccionado.nombre === "DOCTOR" && !form.idDoctor) {
          throw new Error("Debes indicar el id del doctor asociado.");
        }
        ...
        await registrarUsuario({
          ...
          idDoctor: rolSeleccionado.nombre === "DOCTOR" ? Number(form.idDoctor) : null,
        });
      }
  ...
  async function handleToggleActivo(usuario) {
    try {
      if (usuario.activo) {
        ...
        await reactivarUsuario(usuario.idUsuario);
      }
      await cargar();
    } catch (err) {
      setError(extraerMensajeError(err));
              ...
              <tbody className="divide-y divide-slate-100">
                {usuariosPaginados.map((usuario) => (
                  <tr key={usuario.idUsuario} className="hover:bg-slate-50">
                    <td className="px-4 py-3 text-slate-800">{usuario.correo}</td>
                    <td className="px-4 py-3 text-slate-600">{usuario.nombreRol}</td>
                    <td className="px-4 py-3 text-slate-600">
                      {usuario.idDoctor ? `#${usuario.idDoctor}` : "-"}
                    </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!editando && (
            <>
              ...
              <Select
                ...
                required
                value={form.idRol}
                disabled={rolesPermitidos.length === 0}
                onChange={(e) => setForm({ ...form, idRol: e.target.value })}
              >
                {rolesPermitidos.length === 0 ? (
                  <option value="">No hay roles RECEPCIONISTA o DOCTOR activos</option>
                ) : (
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button
              type="submit"
              disabled={guardando || (!editando && rolesPermitidos.length === 0)}
            >
